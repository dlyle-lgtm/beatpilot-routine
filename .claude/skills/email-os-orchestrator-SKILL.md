...
6. Klaviyo build — for every campaign with `qa_status = 'passed'`:
   a. Create or update the campaign in Klaviyo (audience/segment, subject line,
   ...
   b. Read `email_os_config.email_auto_send_enabled` **fresh, every run — never
   ...
   c. If any Klaviyo call fails or returns an unexpected state, stop for that
