...
**Learn the real product before advertising it**: never invent or guess at feature names, audience specifics, or pricing. Before generating any ad, read the actual product documentation in the BeatPilot-Info Drive folder (`1KF05eVO3FVotXQ5oD_KgnSRMTrjwEQ41`), specifically:
- `BeatPilot_Producer_Benefits_Guide.docx` — maps every real feature to a concrete producer benefit; use this to pick which feature to lead an ad with and why it matters
- `BeatPilot — Full Feature Audit.docx` — the actual, current feature inventory (Opportunity Engine, Confidence Score, Market Imbalance, Upload Timing Heatmap, Type Beat Leaderboard, Thumbnail Studio/Analyzer, My Studio, Shorts Signals/Outlier Score, AI Title/Description/Tags Generator) — this is the only source of truth for feature names
- `BeatPilot_Target_Demographic_Customer_Profile.docx` — the real audience profile (2-10 years producing experience, established channels roughly 500-100k subscribers, existing revenue from AdSense/BeatStars/Airbit/Payhip/licensing) to ground copy in specifics, not the generic 18-34 shorthand alone
- `BeatPilot_Paid_Advertising_Strategy.docx` — existing advertising strategy and positioning already worked out, so new ads build on it rather than reinvent it

*Use real, specific features — never invented ones or generic trust badges*: pull the feature name straight from the Full Feature Audit (e.g. Opportunity Engine, Confidence Score, Market Imbalance, Upload Timing Heatmap, Thumbnail Analyzer) — never invent a feature name that sounds plausible but doesn't exist in the product. Never substitute a generic SaaS trust badge ("Private and Secure," "Works Anywhere") for a real, specific capability.

**Use real product screenshots, not invented mockups**: whenever an ad's visual needs to show the actual BeatPilot dashboard, a feature in use, or any other real product UI, pull an actual screenshot from the BeatPilot-Screenshots Drive folder (`1XS-a82qXbLJsYynZIg2W0CpREW3PLXDX`) and use it as a reference/composite image to Higgsfield — don't have Higgsfield invent a plausible-looking but fake dashboard mockup from a text description alone. A fabricated UI risks showing features, numbers, or layouts that don't match the real product.

**Running a job more than once per session**: if asked to run a job multiple times in one sitting (e.g. "run Job 1 three times"), treat each iteration as a full pass — pick, generate, schedule, and log — before starting the next one. Check the relevant log fresh before each iteration, not just once at the start of the batch, so back-to-back iterations don't pick the same reference image, topic, or hook angle as the iteration immediately before it in the same run.
