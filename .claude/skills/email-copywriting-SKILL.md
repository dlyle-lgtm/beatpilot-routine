...
where key in (
  ...
  'klaviyo_list_beatpilot_trial_id'
);
```

Once trial-conversion campaigns are enabled (`lifecycle_data_available = 'true'`), target the confirmed real Klaviyo list `BeatPilotTrial` (id in `klaviyo_list_beatpilot_trial_id`) for `funnel_stage = trial` sends — this is actual list membership, more reliable than the `beatpilot_status` profile field, which is known to go stale after signup (see `email_os_config.klaviyo_beatpilot_status_known_bug`). Still worth confirming whether profiles are ever removed from this list on conversion/cancellation before fully trusting it long-term — same staleness pattern could apply.
...
5. Primary CTA text, matched to funnel_stage:
   ...
   - **all_active** (the current default for market intelligence / product education
     ideas, since real lifecycle segmentation doesn't exist yet — this list includes
     paying customers) → **segment-neutral CTA only.** Never "Start Your Free Trial,"
     never any language assuming the reader hasn't already converted. Use something
     like "See what's trending in your niche" or "Check your dashboard for this,"
     pointed at the product itself, not at signup/upgrade. This is a hard rule, not a
     style preference — getting it wrong means asking existing paying customers to
     start a free trial.
